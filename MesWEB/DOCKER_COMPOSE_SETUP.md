...
`run-compose.bat` を実行すると自動的に `D:\MSSQL\Data` が作成されます。
...
**設定ファイル:**
- コンテナ実行時は `appsettings.Container.json` が使用されます
...
マイグレーションは自動実行されますが、手動で確認・実行する場合：
